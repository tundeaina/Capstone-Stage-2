...
/**
 * Conrec a straightforward method of contouring some surface represented by a regular
 * triangular mesh.
 *
...
public class Conrec {
     ...
     *
     */
    public List<Double[]> contour(double [][] d, int ilb, int iub,
                               int jlb, int jub, double [] x, double [] y, int nc, double [] z) {
        int         m1;
        int         m2;
        int         m3;
        int         case_value;
                        ...
                        if (z[k]>=dmin&&z[k]<=dmax) {
                            ...
                            for (m=1;m<=4;m++) {
                                ...
                                if (case_value!=0) {
                                    ...
                                    segment[0] = Math.round(y1*factor)/factor;
                                    segment[1] = Math.round(x1*factor)/factor;
                                    segment[2] = Math.round(y2*factor)/factor;
                                    segment[3] = Math.round(x2*factor)/factor;
                                    segments.add(segment);

//                                    List<PointUtils.Point2> segment = new ArrayList<PointUtils.Point2>();
//                                    segment.add(pu.new Point2(Math.round(y1*factor)/factor, Math.round(x1*factor)/factor));
//                                    segment.add(pu.new Point2(Math.round(y2*factor)/factor, Math.round(x2*factor)/factor));
//                                    segments.add(segment);

//                                    System.out.println(
//                                            String.format("%f %f %f %f %f"
//                                                    , Math.round(y1*factor)/factor, Math.round(x1*factor)/factor
//                                                    , Math.round(y2*factor)/factor, Math.round(x2*factor)/factor
//                                                    , z[k]));

//                                    System.out.println(
//                                            "INSERT INTO @t values (geography::STLineFromText('LINESTRING("
//                                            +  y1 +" "+ x1 +"," + y2 +" "+ x2 + ")',4326))");
                                }
                            }
                        }
